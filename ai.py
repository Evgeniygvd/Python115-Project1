...
# заполненный только одним символом
def preffered_seq(symbol):
    def preffered_row(collection):
        rows = []
...
def ai_turn(symbol_index):
    ...
    if not field.FIELD[center][center]:
        ...
        if field.FIELD[center][center] == field.SYMBOLS[symbol_index]:
            ...
            y_en, x_en = preffered_seq(field.SYMBOLS[symbol_index - 1])
            # продолжить свой ряд
            y, x = preffered_seq(field.SYMBOLS[symbol_index])
            return y_en, x_en
        # по углам
        else:
            if tuple(chain(*field.FIELD)).count(field.SYMBOLS[symbol_index - 1]) < field.DIM - 1:
                for y in (0, field.DIM - 1):
                    for x in (0, field.DIM - 1):
                        if not field.FIELD[y][x]:
                            return y, x
            # заблокировать противника
            у_en, x_en = preffered_seq(field.SYMBOLS[symbol_index - 1])
            # продолжить свой ряд
            y, x = preffered_seq(field.SYMBOLS[symbol_index])
